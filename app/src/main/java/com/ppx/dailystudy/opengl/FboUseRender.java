 ...
 * @Desc：
 */
public class FboUseRender implements CustomGLSurfaceView.MyGLRender {
    private String TAG = "FboUseRender";
    ...
    private final FboRender yFboRender;
    //顶点坐标
    float[] vertexData = {
            -1f, -1f,
            1f, -1f,
    ...
    public FboUseRender(Context context, int width, int height) {
        this.mContext = context;
        fboWidth = width;
        fboHeight = height;
        yFboRender = new FboRender(context);
        //读取顶点坐标
    ...
    public void onSurfaceCreated() {
        ...
        //加载片元着色器 shader
        String fragmentSource = ShaderUtils.getRawResource(mContext, R.raw.screen_frag);
        //获取源程序
        ...
        Log.d(TAG,"fbo_Width=" + fboWidth + ",fboHeight=" + fboHeight);
        //分配FBO内存大小
        ...
        if (GLES20.glCheckFramebufferStatus(GLES20.GL_FRAMEBUFFER) != GLES20.GL_FRAMEBUFFER_COMPLETE) {
            Log.d(TAG,"fbo bind error !");
        }

        //绑定纹理到 fbo成功后 退出纹理绑定，进入下一环节
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, 0);

        imgTextureId = ShaderUtils.createImageTexture(mContext, R.mipmap.nobb);
