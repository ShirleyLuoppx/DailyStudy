...
import com.gclibrary.util.StrUtil;
...
public class DialogMaker {
     ...
     * @return 显示并返回对话框
     */
    public static Dialog showCommonAlertDialog(Context context, String title, String msg, String[] btns, final DialogCallBack callBack, boolean isCanCancelabel, final boolean isDismissAfterClickBtn, final Object tag, boolean isCenter) {
        ...
        if (null != btns) {
            ...
            if (len > 0) {
                ...
                Button btn;
                View LineView;
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(0, ScrUtils.dpToPx(context, 44));
                params.weight = 1.0f;
                for (int i = 0; i < len; i++) {
                    ...
                    btn.setTextSize(TypedValue.COMPLEX_UNIT_PX, ScrUtils.dpToPx(context, 16));
        ...
        if (StrUtil.isNotBlank(msg)) {
            final int margin = ScrUtils.dpToPx(context, 21.33f);
            ...
            final int margin = ScrUtils.dpToPx(context, 38.67f);
            mParams.topMargin = margin;
        ...
        contentTv.setGravity(isCenter ? Gravity.CENTER : Gravity.LEFT);
        ...
        aWmLp.width = ScrUtils.getScreenWidth(context) - 100;
    ...
    public static void setDialogTextViewContent(Context context, String content, TextView tView) {
        ...
        if (content.contains(NEW_LINE) || content.contains("\n")) {
            ...
            float destWidth = ScrUtils.getContentWidth(content, tView);
            float maxWidth = ScrUtils.dpToPx(context, 235.33f);
...
//        final Dialog dialog = news Dialog(context, R.style.DialogNoTitleRoundCornerStyle);
        View contentView = LayoutInflater.from(context).inflate(R.layout.frame_dialog_wait_common_layout, null);
        Dialog dialog = DialogUtils.dialogCenter(context, contentView,  isCanCancelabel);
        dialog.setOwnerActivity((Activity) context);
        LinearLayout llBg = contentView.findViewById(R.id.ll_bg);
        TextView contentTv = (TextView) contentView.findViewById(R.id.dialog_content_tv);
        Tools.setBgCircle(llBg, 10, R.color.ll_black_80);
...
//        Rotate3dAnimation rotation = news Rotate3dAnimation(0, 360, ViewUtil.dip2px(context, 35), ViewUtil.dip2px(context, 35),
//                0, false);
//        // 动画持续时间500毫秒
...
//        rotation.setInterpolator(news AccelerateDecelerateInterpolator());
//
